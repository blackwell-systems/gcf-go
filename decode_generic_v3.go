...
// DecodeGenericV3 parses GCF v3.0 generic profile text (with inline schemas,
// no-indent attachments, no-prefix inline attachments, shared array schemas).
// Also handles v2.0 input (backwards compatible).
func DecodeGenericV3(input string) (any, error) {
	// Reuse v2 header parsing and body filtering.
	...
	if strings.HasPrefix(trimFirst, "## [") {
		return parseArraySectionV3(contentLines, 0, 0)
	}

	result := make(map[string]any)
	_, err = parseObjectBodyV3(contentLines, 0, 0, result)
	if err != nil {
		return nil, err
	}
	return result, nil
...
func parseObjectBodyV3(lines []string, start, depth int, out map[string]any) (int, error) {
	...
	for i < len(lines) {
		...
		if strings.HasPrefix(content, "## ") {
			...
			if bracketIdx >= 0 {
				...
				arr, consumed, err := parseArrayFromHeaderV3(lines, i, depth, headerContent[bracketIdx:])
				if err != nil {
					return 0, err
				}
				out[name] = arr
			...
			consumed, err := parseObjectBodyV3(lines, i, depth+1, nested)
			if err != nil {
				return 0, err
			}
			out[name] = nested
		...
		if bracketIdx := findInlineArrayBracket(content); bracketIdx > 0 {
			...
			arr, _, err := parseArrayFromHeaderV3(lines, i, depth, rest)
			if err != nil {
				return 0, err
			}
			out[key] = arr
...
func parseArraySectionV3(lines []string, start, depth int) (any, error) {
	first := strings.TrimLeft(lines[start], " ")
	rest := first[3:]
	arr, _, err := parseArrayFromHeaderV3(lines, start, depth, rest)
	return arr, err
}

func parseArrayFromHeaderV3(lines []string, headerLine, depth int, bracketPart string) (any, int, error) {
	...
	if strings.HasPrefix(afterBracket, "{") {
		...
		rows, consumed, err := parseTabularBodyV3(lines, headerLine+1, depth, fields, count, nil)
		if err != nil {
			return nil, 0, err
		}
	...
	items, consumed, err := parseExpandedBodyV3(lines, headerLine+1, depth)
	if err != nil {
		return nil, 0, err
	}
...
func parseExpandedBodyV3(lines []string, start, depth int) ([]any, int, error) {
	indent := strings.Repeat("  ", depth)
	var items []any
	i := start
	...
	for i < len(lines) {
		...
		if strings.HasPrefix(marker, "[") {
			arr, consumed, err := parseArrayFromHeaderV3(lines, i, depth+1, marker)
			if err != nil {
				return nil, 0, err
			}
...
// parseTabularBodyV3 parses tabular rows with v3 extensions:
...
func parseTabularBodyV3(lines []string, start, depth int, fields []string, expectedCount int, parentSharedSchemas map[string][]string) ([]any, int, error) {
	indent := strings.Repeat("  ", depth)
	var rows []any
	i := start
		...
		if rowHasID && len(allAttFields) > 0 {
			...
			for i < len(lines) && len(resolvedAttachments) < len(allAttFields) {
				...
				if strings.HasPrefix(aContent, ".") {
					...
					attNameT, attVal, consumed, parsedFields, err := parseAttachmentV3(lines, i, rest, depth+2, sharedArraySchemas)
					if err != nil {
						return nil, 0, err
					}
...
// parseAttachmentV3 returns (name, value, linesConsumed, parsedFields, error).
// parsedFields is non-nil only when the attachment is a tabular array with explicit {fields}.
func parseAttachmentV3(lines []string, lineIdx int, rest string, depth int, sharedSchemas map[string][]string) (string, any, int, []string, error) {
	...
	if strings.HasPrefix(afterName, "{}") {
		nested := make(map[string]any)
		consumed, err := parseObjectBodyV3(lines, lineIdx+1, depth, nested)
		if err != nil {
	...
	if strings.HasPrefix(afterName, "[") {
		...
		if strings.HasPrefix(afterClose, "{") {
			...
			arr, consumed, err := parseArrayFromHeaderV3(lines, lineIdx, depth, afterName)
			if err != nil {
				return "", nil, 0, nil, err
			}
		...
		if sharedSchemas != nil {
			if sf, ok := sharedSchemas[name]; ok {
				...
				if useShared {
					rows, consumed, err := parseTabularBodyV3(lines, lineIdx+1, depth, sf, count, sharedSchemas)
					if err != nil {
						return "", nil, 0, nil, err
					}
					if count >= 0 && len(rows) != count {
